...
def gumbel_softmax_sample(logits, temperature, dim=1):
    ...
    samples = sample_gumbel(logits.shape, tens_type=type(logits.data))
    if logits.is_cuda:
...
def reset_skill(skill_type, num_skills, num_task, num_agents):
    skill = None
    if skill_type == 'discrete_uniform':
        skill = np.random.multinomial(1, [1. / num_skills] * num_skills, size=num_task)
    elif skill_type == 'gaussian':
        skill = np.random.multivariate_normal(np.zeros(num_skills), np.eye(num_skills), size=num_task)
    elif skill_type == 'cont_uniform':
        skill = np.array([[np.random.uniform(low=-1.0, high=1.0, size=num_skills) for _ in range(num_task)]])
    return skill
