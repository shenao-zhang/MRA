...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        Inputs:
            ...
            latent_dims (list of ints): latent space dims
        """
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.relation_buffs.append(np.zeros((max_steps, 1 * (self.num_agents-1)), dtype=np.float32))
    ...
    def push(self, observations, actions, rewards, next_observations, dones, relation):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.relation_buffs[agent_i] = np.roll(self.relation_buffs[agent_i], rollover, axis=0)
        ...
        for agent_i in range(self.num_agents):
            ...
            self.relation_buffs[agent_i][self.curr_i:self.curr_i + nentries] = relation[agent_i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                [cast(self.relation_buffs[i][inds]) for i in range(self.num_agents)])
